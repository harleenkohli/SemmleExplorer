...
namespace InsufficientKeySize
{
    class MainCrypto
    {
        ...
        static public byte[] EncryptWithRSA(byte[] plaintext, RSAParameters key) {
             try {
                RSACryptoServiceProvider rsa = new RSACryptoServiceProvider(512); // VIOLATION
        ...
        static public byte[] EncryptWithRSA2(byte[] plaintext, RSAParameters key) {
             try {
                RSACryptoServiceProvider rsa = new RSACryptoServiceProvider(); // VIOLATION
                rsa = new RSACryptoServiceProvider(1024); // NON-VIOLATION
                rsa.ImportParameters(key);
                return rsa.Encrypt(plaintext, true);
            }
            catch(CryptographicException e) {
        ...
        static public byte[] EncryptWithDSA(byte[] plaintext, DSAParameters key) {
             try {
                DSACryptoServiceProvider dsa = new DSACryptoServiceProvider(2048); // NON-VIOLATION
        ...
        static public byte[] EncryptWithDSA2(byte[] plaintext, DSAParameters key) {
             try {
                DSACryptoServiceProvider dsa = new DSACryptoServiceProvider(); // VIOLATION
                dsa = new DSACryptoServiceProvider(1024); // NON-VIOLATION
                dsa.ImportParameters(key);
                return dsa.SignData(plaintext);
            }
            catch(CryptographicException e) {
        ...
        string HashAlg)
    {
        ...
        try
        {
            ...
            using (DSACryptoServiceProvider DSA = new DSACryptoServiceProvider(1024)) // NON-VIOLATION
            {
        ...
        DSAParameters DSAKeyInfo, string HashAlg)
    {
        ...
        {
            // Create a new instance of DSACryptoServiceProvider.
            using (DSACryptoServiceProvider DSA = new DSACryptoServiceProvider())  // VIOLATION
            {
                // Import the key information.
                DSA.ImportParameters(DSAKeyInfo);
    ...
    public static void Main2()
    {
        ...
        DESCryptoServiceProvider key = new DESCryptoServiceProvider();  // VIOLATION
   ...
   // Decrypt the byte array.
    public static string Decrypt(byte[] CypherText, SymmetricAlgorithm key)
    {
        ...
        public static void RC2()
        {
            ...
            rc2CSP.EffectiveKeySize = KEY_SIZE; // VIOLATION
